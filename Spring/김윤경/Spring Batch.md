...
```
-> 700건이면 SQL 700번 실행 -> 네트워크 700번 왕복

```
jdbcTemplate.batchUpdate(
   "INSERT INTO settlement (store_id, amount) VALUES (?, ?)",
   batchArgs
);
```
-> batchUpdate를 사용하면 빠르다.
...
public class SettlementItemWriter implements ItemWriter<Settlement> {
    ...
    public void write(List<? extends Settlement> items) {
        ...
        String sql = """
            ...
            (store_id, settlement_date, total_amount, order_count)
            VALUES (?, ?, ?, ?)
...
```
-> chunk 단위로 들어온 정산 결과를 한 번에 저장한다. 
...
- rewriteBatchedSatements=true :
  <br> batchUpdate()로 여러 insert를 보낼 때, 드라이버가 SQL를 “진짜 한 번의 대량 insert”로 바꿔서 보내주는 기능
...
배치에 1000개가 있으면 내부적으로

```
INSERT INTO settlement VALUES (1, 10000);
INSERT INTO settlement VALUES (2, 20000);
INSERT INTO settlement VALUES (3, 15000);
...
(1000번)
```
-> 느림

- rewriteBatchedStatements=true 있을 때
...
(1000개 한번에)
```
-> 네트워크 왕복 1번, SQL 실행 1번, InnoDB 처리 효율 극대화
-> 성능이 보통 5배~30배 이상 빨라짐

```
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/yourdb?rewriteBatchedStatements=true
```
-> Spring Boot와 MySQL 기준으로 기존 옵션 뒤에 &rewriteBatchedStatements=true 로 붙인다.
